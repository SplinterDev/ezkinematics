...
# to avoid creating a new Point2D every loop, reuse this one:
ORIGIN = Point2D()
...
class Application:
    ...
    def draw_arm(self, arm, color='black'):
        # p1 = Point2D(r=arm.lengths[0], a=arm.t1)
        # p2 = Point2D(r=arm.lengths[1], a=arm.t2+arm.t1) + p1
        # p3 = Point2D(r=arm.lengths[2], a=arm.t3+arm.t2+arm.t1) + p2

        # self.ez.line(Point2D(), p1, color=color)
        # self.ez.point(p1)
        ...
        for i, p in enumerate(points):
            if i == 0:
                self.ez.line(Point2D(), p, color=color)
            else:
                self.ez.line(points[i-1], p, color=color)
            self.ez.point(p)


        # print("Not drawing yet!")
    ...
    #     self.ez.text(val_text, Point2D(10,10), onScreen=True)
    #     # passo atual
    #     val = self.trajectory.curr_idx()
    #     val_text = "Current step..{0}".format(val)
    #     self.ez.text(val_text, Point2D(10,20), onScreen=True)
    #     # erro medio
    #     val = self.mse()
    #     val_text = "Avg. error....{0}".format(val)
    #     self.ez.text(val_text, Point2D(10,30), onScreen=True)
    ...
    #             Point2D(self.traj_step * 10, mse*1000)
    #         )
    #     elif self.loop == TEST_NETWORK:
